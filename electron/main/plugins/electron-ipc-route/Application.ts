...
export default class {
    ...
    constructor(private rootName:string) {
        ipcMain.handle(this.rootName, (event, sourceStr: string) => {
            ...
            // 执行中间件
            const matchMiddlewares = this.middlewares.filter(mw => mw.route === sourceData.method || !mw.route);
            let index = 0;
            const next = () => {
                if (index < matchMiddlewares.length - 1) {
                    ...
                    // 找到符合条件的路由程序
                    const matchRouteItem = this.routeListeners.find(item => item.method === sourceData.method);
                    if (!matchRouteItem) {
                        return {
                            message: 'unknown'
                        };
    ...
    use(...params: any[]) {
        if (params.length === 2 && params[1].isRoute) {
            ...
            this.routeListeners.concat(listeners.map((item: {
                method: string;
                listener: RouteListener
            }) => ({
